...
app_ctx = create_ctx('app', {'data': 0})

print(app_ctx['data'])
# 0

app_ctx_1 = app_ctx.new({'data': 1})
with context('app', app_ctx_1):
    print(app_ctx['data'])
    # Exception: app_ctx not frozen
...
app_ctx_2 = app_ctx.new()
app_ctx.set({'data': 2})
app_ctx.freeze()
with context('app', app_ctx_2):
    print(app_ctx['data'])
    # 2
...
def custom_freezer(data):
    if isinstance(data, CustomData):
        return FrozenData(CustomData)
app_ctx = create_ctx('app', {'data': CustomData}, freezer=custom_freezer)

worker_store = create_ctx('worker', {'data': 0}, require_frozen=False):
"""
...
class Context(Mapping, metaclass=Singleton):
    ...
    * We don't want people manually messing with the threading.local() object as they might
    ...
    """
    _store = create_context('_context', {})
    _default = None
...
def create_context(context_name, context_data):
    ctxt = contextvars.ContextVar(context_name)
    ctxt.set(context_data)
